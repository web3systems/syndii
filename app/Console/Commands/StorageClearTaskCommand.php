...
class StorageClearTaskCommand extends Command
{
    ...
    public function processImages()
    {
        ...
        if ($days != -1) {
           ...
           if ($images) {
                foreach($images as $row) {
                    ...
                    switch ($row->storage) {
                        ...
                        case 'dropbox':
                            ...
                            break;
                        case 'r2':
                            if (Storage::disk('r2')->exists($row->image_name)) {
                                Storage::disk('r2')->delete($row->image_name);
                            }
                            break;
                        default:
                            # code...
                ...
                Log::info('Expired images are cleared successfully');
           }
        ...
        if ($days != -1) {
           ...
           if ($voiceovers) {
            foreach($voiceovers as $row) {
                ...
                switch ($row->storage) {
                    case 'local':
                        if (Storage::disk('public')->exists($row->file_name)) {
                            Storage::disk('public')->delete($row->file_name);
                        }
                        break;
                    case 'aws':
                    ...
                    case 'dropbox':
                        ...
                        break;
                    case 'r2':
                         if (Storage::disk('r2')->exists($row->file_name)) {
                             Storage::disk('r2')->delete($row->file_name);
                         }
                         break;
                    default:
                        # code...
    ...
    public function processVideos()
    {
        ...
        if ($days != -1) {
           $videos = VideoTextResult::where('created_at', '<', Carbon::now()->subDays($days))->get();
           ...
           if ($videos) {
            foreach($videos as $row) {
                ...
                switch ($row->storage) {
                    case 'local':
                        if (Storage::disk('public')->exists($row->video)) {
                    ...
                    case 'dropbox':
                        ...
                        break;
                    case 'r2':
                        if (Storage::disk('r2')->exists($row->video)) {
                            Storage::disk('r2')->delete($row->video);
                        }
                        if (Storage::disk('r2')->exists($row->image)) {
                            Storage::disk('r2')->delete($row->image);
                        }
                        break;
                    default:
                        # code...
