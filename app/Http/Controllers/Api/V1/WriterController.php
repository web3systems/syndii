...
class WriterController extends Controller
{
     ...
     *      tags={"AI Writer"},
     *      summary="Get list of original and custom templates",
     *      description="Get list of original and custom templates (both created by the admin and the requesting user).",
     *      security={{ "passport": {} }},
     *      @OA\Response(
     *          response=200,
     ...
     *              @OA\Property(property="original_templates", type="object", description="Eloguent object for list of original templates that come with the script"),
     *              @OA\Property(property="custom_templates", type="object", description="Eloguent object for list of custom templates created by the user"),
     *              @OA\Property(property="public_custom_templates", type="object", description="Eloguent object for list of public custom templates created by admin"),
     *          ),
     *      ),
     *      @OA\Response(
     *          response=401,
    ...
    */
    ...
    {   
        ...
        $original_templates = Template::where('status', true)->orderBy('group', 'asc')->get();   
        $custom_templates = CustomTemplate::where('user_id', auth()->user()->id)->where('type', 'private')->where('status', true)->orderBy('group', 'asc')->get();  
        $public_custom_templates = CustomTemplate::where('type', '<>', 'private')->where('status', true)->orderBy('group', 'asc')->get();  
        
        return response()->json(['categories' => $categories, 'original_templates' => $original_templates, 'custom_templates' => $custom_templates, 'public_custom_templates' => $public_custom_templates], 200); 
    }
     ...
     *
     * @OA\Post(
     *      path="/api/v1/template/generate",
     *      operationId="generateTextTemplate",
     *      tags={"AI Writer"},
     *      summary="Generate AI Text",
     *      description="Generate AI Text based on the prompt of the selected template",
     *      security={{ "passport": {} }},
     *      @OA\Parameter(
     *          name="template_code",
     *          in="path",
     *          description="Template code of the current selected template that is being used to generate text",
     *          required=true,
     ...
     *          name="document_id",
     *          in="path",
     *          description="Include the document_id that was created dynamically when you selected a template to use",
     *          required=true,
     ...
     *          name="model",
     *          in="path",
     *          description="Model that you want to use to generate AI Text",
     *          required=true,
     *          @OA\Schema(type="string", example="gpt-4o"),
     *      ),
     *      @OA\Parameter(
     *          name="template_fields",
     *          in="path",
     *          description="These are all fields related to the selected template. They come with the template when you first open it. Make sure to showcase all fields in the user UI so that their inputs are captured in the prompts properly.",
     *          required=true,
     *      ),
     *      @OA\Parameter(
     *          name="language",
     *          in="path",
     *          description="Selected language to generate text",
     *          required=true,
     ...
     *          name="temperature",
     *          in="path",
     *          description="Creativity level of the response, can ben between 0 - 1.0 values only. The higher the value to more creative is the response.",
     *          required=true,
     *          @OA\Schema(type="float", example="1.0"),
     *      ),
     *      @OA\Parameter(
     *          name="max_words",
     *          in="path",
     *          description="Maximum expected words",
     *          required=true,
    ...
    */
	public function generate(Request $request) 
    {
        ...
        return response()->stream(function () use($model, $prompt, $document_id, $max_results, $max_words, $temperature, $language) {
            ...
            $content = Content::where('id', $document_id)->first();
	...
	*/
     ...
     *
     * @OA\Post(
     *      path="/api/v1/template/save",
     *      operationId="templateSave",
     *      tags={"AI Writer"},
     *      summary="Save the generated text",
     *      description="Save the generated text result for a template",
     *      security={{ "passport": {} }},
     *      @OA\Parameter(
     *          name="document_id",
     *          in="path",
     ...
     *          name="text",
     *          in="path",
     *          description="Response text to save",
     *          required=true,
     *          @OA\Schema(type="string", example="Template response text"),
     *      ),
     *      @OA\Response(
     *          response=201,
     *          description="Successful operation",
    ...
    */
	public function save(Request $request) 
    {
        ...
        $document = Content::where('id', request('document_id'))->first(); 
        ...
        if ($document) {
            if ($document->user_id == Auth::user()->id){
                ...
                return response()->json(['message' => __("Results saved successfully")], 201);
    
            } else{
                return response()->json(['error' => __("Unauthorized request")], 405);
            }  
        } else {
    ...
    /**
     * Set AI Template to be favorite
     *
     * @OA\Post(
     *      path="/api/v1/template/favorite",
     *      operationId="templateFavorite",
     *      tags={"AI Writer"},
     *      summary="Set AI Template as favorite",
     *      description="Set AI Template to be favorite based on template code.",
     *      security={{ "passport": {} }},
     *      @OA\Parameter(
     *          name="template_code",
     *          in="path",
     *          description="Template code that you want to set as favorite",
     *          required=true,
     *          @OA\Schema(type="string", example="TKDRY"),
     *      ),
     *      @OA\Response(
    ...
    */
    ...
    {
        if($request->template_code == null) {
            return response()->json(['error' => __('Template code is missing.')], 412);
        }

        if (strlen(request('template_code')) < 6) {
            $template = Template::where('template_code', request('template_code'))->first(); 
        } else {
            $template = CustomTemplate::where('template_code', request('template_code'))->first(); 
        }
        ...
        if ($template) {
            ...
            if ($favorite) {
                ...
                return response()->json(['message' => 'AI Template has been removed from favorite list'], 201); 
                ...
                return response()->json(['message' => 'AI Template has been set as favorite'], 201);
            }  
        } else {
     ...
     *
     * @OA\Get(
     *      path="/api/v1/template/{template_code}",
     *      operationId="templateViewOriginal",
     *      tags={"AI Writer"},
     *      summary="View template details",
     *      description="View template details to initiate a text generation process",
     *      security={{ "passport": {} }},
     *      @OA\Parameter(
     *          name="template_code",
     *          in="path",
     *          description="Template code that you want to view",
     *          required=true,
     *          @OA\Schema(type="string", example="TKDRY"),
     *      ),
     *      @OA\Response(
     ...
     *          @OA\JsonContent(
     *              type="object",
     *              @OA\Property(property="template", type="object", description="Eloguent object for selected template"),
     *              @OA\Property(property="template_fields", type="json", description="Input fields related to the selected template"),
     *              @OA\Property(property="workbooks", type="object", description="Eloguent object for user workbooks"),
     *              @OA\Property(property="languages", type="object", description="List of languages for the templates"),
     *              @OA\Property(property="document_id", type="integer", description="Document ID, needed for saving results permanently"),
     *          ),
     *      ),
     *      @OA\Response(
     *          response=401,
    ...
    */
    ...
    {   
        ...
        if ($template) {
            ...
            $user = Auth::user();
            $plan_type = ($user->plan_id) ? 'paid' : 'free';

            $content = new Content();
            $content->user_id = Auth::id();
            $content->template_code = $request->template_code;
            $content->template_name = $template->name;
            $content->icon = $template->icon;
            $content->group = $template->group;
            $content->tokens = 0;
            ...
            return response()->json(['languages' => $languages, 'workbooks' => $workbooks, 'template' => $template, 'template_fields' => $fields, 'document_id' => $content->id], 200); 
	...
	*
	*/
	public function preparePrompt(Request $request) 
    {
